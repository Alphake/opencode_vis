...
这里的「任务」指一段可独立概括的工作单元（有相对稳定的主目标与交付焦点），不是整次会话或整个项目。**倾向细切**：同一主题下，只要工作阶段、交付物类型或可单独命名的焦点变了，就应切为新任务段，而不是把整段对话收成一个大任务。

### 常见应切分的阶段（同一项目内也适用）

以下类型通常各自成段；从一种转到另一种时，输出 `task_switched = true`：

| 阶段类型 | 标题示例 | 说明 |
|----------|----------|------|
| **探索搜集** | `调研插件存储方案`、`搜集竞品 API` | 以信息收集、阅读、对比为主，交付物是认知/选项/材料 |
| **方案规划** | `定 Tab 切换方案`、`规划数据流架构` | 以选型、设计、拆步骤、列验收标准为主，交付物是方案而非成品 |
| **Demo / 原型实现** | `做图表 Demo`、`搭最小可跑原型` | 以快速验证、粗粒度实现为主，范围偏整体验证 |
| **明确功能实现** | `修 Tab 切换 bug`、`加导出 CSV 按钮` | 目标非常具体、可单独验收的一个功能点或修复 |
| **评审 / 验收 / 收尾** | `对照清单验收`、`整理 PR 描述` | 与实现阶段不同的检查、文档或交付动作 |

**不要**把「探索 → 规划 → Demo → 某个具体功能」合并成一条任务；每一跳都是切换。

在以下情况输出 `task_switched = true`：

- 当前输入开启新目标、新交付物、新问题域或新工作对象。
- 当前输入进入上表中的**另一工作阶段**（例如从探索搜集到方案规划，从 Demo 到某个明确功能的细化实现）。
- 当前输入的主诉求与先前段不同：即便题材相关，完成标准、要产出的结果或当下要推进的事已经换了。
- 当前输入指向一个**可单独命名、可单独验收**的功能点、模块或修复，且与先前段的交付焦点不同。
- 当前输入明确要求停止/放弃上一任务并做别的事。
- 当前输入与累积的先前输入之间没有「继续、纠正或紧接扩展同一未完成交付」的关系。

在以下情况输出 `task_switched = false`：

- 当前输入只是为**同一未完成交付、同一工作阶段**补充细节、约束、验收标准或格式，主诉求未变。
- 当前输入是对**刚在做的同一件事**做小幅纠正、重做、澄清或追问结果。
- 当前输入明显是上一句的直接续写，且仍在同一阶段、同一交付物上推进。

拿不准时：若当前输入可以写成与先前段**阶段或交付物明显不同**的独立标题，倾向 `true`；仅当几乎只能写成「继续上一件事、同一阶段」时才选 `false`。
