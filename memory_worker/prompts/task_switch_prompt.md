...
这里的「任务」指一段可独立概括的工作单元（有相对稳定的主目标与交付焦点），不是整次会话或整个项目。同一主题下，若用户把注意力转到另一件可单独命名的事上，也应视为切换。

在以下情况输出 `task_switched = true`：

- 当前输入开启新目标、新交付物、新问题域或新工作对象。
- 当前输入的主诉求与先前段不同：即便题材相关，完成标准、要产出的结果或当下要推进的事已经换了。比如从方案的调研到实际的执行，从数据的收集到数据分析产出，比如从一个较为宏观的初步demo实现到更明确的某一个特定功能的细化和实现。
- 当前输入明确要求停止/放弃上一任务并做别的事。
- 当前输入与累积的先前输入之间没有「继续、纠正或紧接扩展同一未完成交付」的关系。

在以下情况输出 `task_switched = false`：

- 当前输入只是为**同一未完成交付**补充细节、约束、验收标准或格式，主诉求未变。
- 当前输入是对**刚在做的同一件事**做小幅纠正、重做、澄清或追问结果。
- 当前输入明显是上一句的直接续写且没有换一件可单独命名的事。

拿不准时：若当前输入本身可以写成一条独立的任务标题（与先前段标题会明显不同），倾向 `true`；仅当几乎只能写成「继续上一件事」时才选 `false`。
